...
import {executeFromGovernorMultiSig} from '../../helpers/deployment'
...
const func: DeployFunction = async (hre: HardhatRuntimeEnvironment) => {
  ...
  if ((await read(MasterOracle, 'defaultOracle')) !== defaultOracleAddress) {
    // Note: Do not store but execute immediately because `setup_token_oracles` needs default oracle in place
    await executeFromGovernorMultiSig(hre, MasterOracle, 'updateDefaultOracle', defaultOracleAddress)
  }
}
